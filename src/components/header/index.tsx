            ...
            <div className={style.container}>
                <div className={`${style.container__categories}`}>
                    {this.state.categories.map((c: any) =>
                        <p key={c.name} className={this.props.category.toUpperCase() === c.name.toUpperCase() ? style[`container__categories--active`] : ""} onClick={(e) => this.handle(e)}>{c.name.toUpperCase()}</p>)}
                </div>
                <a href={"/cart"}><img src={bag} style={{ maxWidth: '3vw' }}></img></a>
                <div className={style.container__buttons}>
                    <div >
                        <label>{this.state.currency}
                            <select className={`${this.state.dispOps ? '' : style[`container__buttons--inactive`]}`} multiple={true}  onChange={e => this.setState({ currency: e.target.value })}>
